...
| `dod` | `ci, build, lint, tests` all `true`, plus `bpa, tmdlBreaking` **only when the repo has a semantic model** (a `.tmdl` / `.bim` / `.pbism` file) | The definition of done the loop drives toward ("everything passes"). `bpa` and `tmdlBreaking` are semantic-model gates, so the default is derived from the repo (#475): a repo with no model gets neither, a repo with one gets both, and a repo that could not be scanned keeps both (could-not-look never waives a gate). A value already on disk always wins. Which enabled gates a given *diff* owes is decided separately by `Expert-WorkClass.Get-ApplicableDodGates`; the ones it does not owe are recorded in the evidence as `N/A`, never `PASS` (see `SKILL.md` → Evidence). |
