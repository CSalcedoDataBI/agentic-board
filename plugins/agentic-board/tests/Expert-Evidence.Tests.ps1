...
Describe 'Format-EvidenceLinkStub - one source of truth, two pointers (#570)' {
    It 'carries the marker, the summary and the linked path' {
        $s = Format-EvidenceLinkStub -Issue 42 -Results @(@{ result = 'PASS' }, @{ result = 'PASS' }, @{ result = 'FAIL' }) -Repo 'o/r' -Branch 'issue-42-x'
        $s | Should -Match '\[abios-evidence\]'
        $s | Should -Match '2 passed / 1 failed'
        $s | Should -Match 'https://github.com/o/r/blob/issue-42-x/evidence/42.md'
    }
